...
int main(int argc, char *argv[])
{
	...
	//~ printf("%s see \n", ch);
	//~ int i;
//~ // you CANNOT do char* ch[5], do char* ch; ch=malloc(5);
	//~ printf("\n\n");
...
//~ char* letter_ch;
	//~ letter_ch=(char *)malloc(5);
	//~ char *name;
	//~ name=(char *) malloc(sizeof(fullpath)+20);
	//~ int num=0;
	//~ for(i=0; i<12; i++){
		//~ snprintf(letter_ch, 5, "%d", num); // this is actually stealing memory
		//~ strcpy(name, fullpath);
		//~ strcat(name, "fusedata.");
		//~ strcat(name, letter_ch);
		//~ printf("%s\n",name);
	...
	//~ }
		...
		//~ free(name);
		//~ 
				// need context so it writes to this file path, and leave off the / so it writes to parent dir 
		// for now
	//	char* name[4096];
	/*	strcat(name, fuse_get_context()->private_data);
		strcat(name, "fusedata.");
		strcat(name, letter_ch);*/
	//	strcat(name, '\0');
	//	FILE* fd=fopen("fusedata.z","w+");
