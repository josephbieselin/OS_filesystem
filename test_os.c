...
/*
fusermount -u ~/OS/filesystem/fuse/mount;
gcc -Wall -g ~/OS/filesystem/test_os.c `pkg-config fuse --cflags --libs` -o jabFS;
./jabFS fuse/mount;
ls -al fuse/mount
*/
...
static void* KATZ_init(struct fuse_conn_info *conn)
{
	int letter='0';
 	char* buf;
 	buf=(char *)malloc(BLOCK_SIZE); // 4096 bytes to a file
	...
	for(i=0; i<1; i++){
		...
		chdir(fuse_get_context()->private_data);
		chdir("..");
		getcwd(name, 255*4*4);
		// strcpy(name, fuse_get_context()->private_data);
		// strcat(name, "fusedata.");
		// strcat(name, letter_ch);
		char* file = malloc(MAX_PATH * sizeof(char));
		file = "fusedata.";
		strcat(file, letter_ch);
